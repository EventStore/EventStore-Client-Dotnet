...
namespace EventStore.Client {
	public abstract class EventStoreClientFixtureBase<T> : IAsyncLifetime where T : IDisposable {
		...
		protected EventStoreClientFixtureBase(EventStoreClientSettings? clientSettings, string[]? env = null) {
		...
		protected abstract T CreateClient(EventStoreClientSettings settings);
		...
		protected class EventStoreTestServer : IAsyncDisposable {
			...
			public Task Started => _container.Started;

			public EventStoreTestServer(string[]? env) {
				...
				_container = new DockerContainer("eventstore/eventstore", tag, async ct => {
					try {
						using var response = await _httpClient.GetAsync("/web/index.html");
					...
					Env = new List<string>(env ?? Array.Empty<string>()) {
						"EVENTSTORE_DEV=true"
					},
