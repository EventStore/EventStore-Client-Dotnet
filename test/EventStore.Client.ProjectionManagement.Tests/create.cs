...
public class @create : IClassFixture<create.Fixture> {
	...
	public create(Fixture fixture) => _fixture = fixture;

	[Fact]
	public async Task one_time() =>
		await _fixture.Client.CreateOneTimeAsync("fromAll().when({$init: function (state, ev) {return {};}});", userCredentials: TestCredentials.Root);
