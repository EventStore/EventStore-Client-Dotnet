...
namespace EventStore.Client {
	...
	public class read_enumeration_tests : IClassFixture<read_enumeration_tests.Fixture> {
		...
		public read_enumeration_tests(Fixture fixture) {
			_fixture = fixture;
		}

		[Fact]
		public async Task all_referencing_messages_twice_does_not_throw() {
			var result = _fixture.Client.ReadAllAsync(Direction.Forwards, Position.Start, 32,
		...
		public async Task referencing_messages_twice_does_not_throw() {
			var result = _fixture.Client.ReadStreamAsync(Direction.Forwards, "$users", StreamPosition.Start, 32,
				userCredentials: TestCredentials.Root);
