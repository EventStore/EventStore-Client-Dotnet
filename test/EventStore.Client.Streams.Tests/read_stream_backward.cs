...
public class read_stream_backward : IClassFixture<read_stream_backward.Fixture> {
	...
	public read_stream_backward(Fixture fixture) => _fixture = fixture;
	...
	public async Task count_le_equal_zero_throws(long maxCount) {
		...
		var ex = await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
			() =>
				_fixture.Client.ReadStreamAsync(Direction.Backwards, stream, StreamPosition.Start, maxCount)
					.ToArrayAsync().AsTask()
		);
	...
	public async Task be_able_to_read_single_event_from_arbitrary_position() {
		...
		var actual = await _fixture.Client.ReadStreamAsync(Direction.Backwards, stream, new(7), 1)
			.Select(x => x.Event)
			.SingleAsync();
	...
	public async Task be_able_to_read_from_arbitrary_position() {
		...
		var actual = await _fixture.Client.ReadStreamAsync(Direction.Backwards, stream, new(3), 2)
			.Select(x => x.Event)
			.ToArrayAsync();
