...
namespace EventStore.Client {
	internal class ComparableAssertion : CompositeIdiomaticAssertion {
		...
		private class ImplementsIComparableCorrectlyAssertion : IdiomaticAssertion {
			public override void Verify(Type type) =>
				Assert.False(type.ImplementsGenericIComparable() && !type.ImplementsIComparable(), 
					$"The type {type} implemented IComparable<T> without implementing IComparable.");
		}

		private class SameValueComparableAssertion : IdiomaticAssertion {
			...
			public override void Verify(Type type) {
				...
				Assert.True(type.InvokeGreaterThanOrEqualOperator(instance, instance),
					$"The type {type} did not implement >= correctly, should be true for the same instance.");
				Assert.False(type.InvokeGreaterThanOperator(instance, instance),
					$"The type {type} did not implement > correctly, should be false for the same instance.");
				Assert.True(type.InvokeLessThanOrEqualOperator(instance, instance),
					$"The type {type} did not implement <= correctly, should be true for the same instance.");

				Assert.False(type.InvokeLessThanOperator(instance, instance),
					$"The type {type} did not implement <= correctly, should be true for the same instance.");
		...
		private class DifferentValueComparableAssertion : IdiomaticAssertion {
			private readonly ISpecimenBuilder _builder;

			public DifferentValueComparableAssertion(ISpecimenBuilder builder) =>
			...
			public override void Verify(Type type) {
				...
				var ex = Assert.Throws<ArgumentException>(() => {
					try {
						type.InvokeCompareTo(instance, new object());
				...
				if (compareToGeneric < 0) {
					Assert.False(type.InvokeGreaterThanOrEqualOperator(instance, other),
						$"The type {type} did not implement >= correctly, should be false for different instances.");
					Assert.False(type.InvokeGreaterThanOperator(instance, other),
						$"The type {type} did not implement > correctly, should be false for different instances.");
					Assert.True(type.InvokeLessThanOrEqualOperator(instance, other),
						$"The type {type} did not implement <= correctly, should be true for different instances.");
					Assert.True(type.InvokeLessThanOperator(instance, other),
						$"The type {type} did not implement <= correctly, should be true for different instances.");
				} else {
					Assert.True(type.InvokeGreaterThanOrEqualOperator(instance, other),
						$"The type {type} did not implement >= correctly, should be true for different instances.");
					Assert.True(type.InvokeGreaterThanOperator(instance, other),
						$"The type {type} did not implement > correctly, should be true for different instances.");
					Assert.False(type.InvokeLessThanOrEqualOperator(instance, other),
						$"The type {type} did not implement <= correctly, should be false for different instances.");
					Assert.False(type.InvokeLessThanOperator(instance, other),
						$"The type {type} did not implement <= correctly, should be false for different instances.");
				}
			}
		}
