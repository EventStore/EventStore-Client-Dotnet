...
partial class EventStorePersistentSubscriptionsClient {
	...
	public async Task CreateToStreamAsync(
		...
		await CreateInternalAsync(
				streamName, groupName, null, settings, deadline, userCredentials,
				cancellationToken
			)
	...
	public async Task CreateToAllAsync(
		...
		await CreateInternalAsync(
				SystemStreams.AllStream, groupName, eventFilter, settings, deadline,
	...
	public async Task CreateToAllAsync(
		...
		await CreateInternalAsync(
				SystemStreams.AllStream, groupName, null, settings, deadline, userCredentials,
				cancellationToken
			)
			.ConfigureAwait(false);
	...
	async Task CreateInternalAsync(
		...
		using var call = new PersistentSubscriptions.PersistentSubscriptions.PersistentSubscriptionsClient(channelInfo.CallInvoker).CreateAsync(
			new CreateReq {
