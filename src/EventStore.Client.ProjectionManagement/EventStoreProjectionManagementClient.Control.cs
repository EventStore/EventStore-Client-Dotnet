...
public partial class EventStoreProjectionManagementClient {
	...
	public async Task EnableAsync(
		...
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).EnableAsync(
			new EnableReq {
	...
	public async Task ResetAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).ResetAsync(
			new ResetReq {
	...
	public async Task RestartSubsystemAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).RestartSubsystemAsync(
			new Empty(),
			EventStoreCallOptions.CreateNonStreaming(Settings, deadline, userCredentials, cancellationToken)
		);
	...
	async Task DisableInternalAsync(
		string name, bool writeCheckpoint, TimeSpan? deadline,
		UserCredentials? userCredentials, CancellationToken cancellationToken
	) {
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).DisableAsync(
			new DisableReq {
