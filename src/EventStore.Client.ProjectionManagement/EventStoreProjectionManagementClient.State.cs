...
public partial class EventStoreProjectionManagementClient {
	...
	async ValueTask<Value> GetResultInternalAsync(
		...
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).ResultAsync(
			new ResultReq {
	...
	async ValueTask<Value> GetStateInternalAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Projections.Projections.ProjectionsClient(channelInfo.CallInvoker).StateAsync(
			new StateReq {
		...
		public override void Write(Utf8JsonWriter writer, Value value, JsonSerializerOptions options) {
			switch (value.KindCase) {
				...
				case Value.KindOneofCase.ListValue:
					...
					foreach (var item in value.ListValue.Values) Write(writer, item, options);
