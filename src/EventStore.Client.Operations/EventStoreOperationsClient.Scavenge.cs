...
public partial class EventStoreOperationsClient {
	...
	public async Task<DatabaseScavengeResult> StartScavengeAsync(
		...
		if (startFromChunk < 0) throw new ArgumentOutOfRangeException(nameof(startFromChunk));

		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Operations.Operations.OperationsClient(channelInfo.CallInvoker).StartScavengeAsync(
	...
	public async Task<DatabaseScavengeResult> StopScavengeAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		var result = await new Operations.Operations.OperationsClient(channelInfo.CallInvoker).StopScavengeAsync(
				new StopScavengeReq {
