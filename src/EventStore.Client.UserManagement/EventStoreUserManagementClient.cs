...
public sealed class EventStoreUserManagementClient : EventStoreClientBase {
	static readonly Dictionary<string, Func<RpcException, Exception>> ExceptionMap =
		new() {
			[Constants.Exceptions.UserNotFound] = ex => new UserNotFoundException(ex.Trailers.First(x => x.Key == Constants.Exceptions.LoginName).Value)
		};
	...
	public async Task CreateUserAsync(
		...
		using var call = new Users.Users.UsersClient(channelInfo.CallInvoker).CreateAsync(
			new CreateReq {
	...
	public async Task<UserDetails> GetUserAsync(
		string loginName, TimeSpan? deadline = null,
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Users.Users.UsersClient(channelInfo.CallInvoker).Details(
			new DetailsReq {
	...
	static UserDetails ConvertUserDetails(DetailsResp.Types.UserDetails userDetails) =>
		new(
			userDetails.LoginName, userDetails.FullName, userDetails.Groups.ToArray(),
	...
	public async Task DeleteUserAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		var call = new Users.Users.UsersClient(channelInfo.CallInvoker).DeleteAsync(
			new DeleteReq {
	...
	public async Task EnableUserAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Users.Users.UsersClient(channelInfo.CallInvoker).EnableAsync(
			new EnableReq {
	...
	public async Task DisableUserAsync(
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		var call = new Users.Users.UsersClient(channelInfo.CallInvoker).DisableAsync(
			new DisableReq {
	...
	/// <param name="cancellationToken"></param>
	/// <returns></returns>
	public async IAsyncEnumerable<UserDetails> ListAllAsync(
	...
	public async Task ChangePasswordAsync(
		string loginName, string currentPassword, string newPassword,
		TimeSpan? deadline = null, UserCredentials? userCredentials = null,
		CancellationToken cancellationToken = default
	) {
		if (loginName == null) throw new ArgumentNullException(nameof(loginName));
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		using var call = new Users.Users.UsersClient(channelInfo.CallInvoker).ChangePasswordAsync(
	...
	public async Task ResetPasswordAsync(
		string loginName, string newPassword,
		TimeSpan? deadline = null, UserCredentials? userCredentials = null,
		CancellationToken cancellationToken = default
	) {
		if (loginName == null) throw new ArgumentNullException(nameof(loginName));
		...
		var channelInfo = await GetChannelInfo(cancellationToken).ConfigureAwait(false);
		var call = new Users.Users.UsersClient(channelInfo.CallInvoker).ResetPasswordAsync(
