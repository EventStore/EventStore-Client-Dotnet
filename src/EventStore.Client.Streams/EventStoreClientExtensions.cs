...
public static class EventStoreClientExtensions {
	static readonly JsonSerializerOptions SystemSettingsJsonSerializerOptions = new() {
		Converters = {
	...
	public static Task SetSystemSettingsAsync(
		...
		return client.AppendToStreamAsync(
			SystemStreams.SettingsStream, StreamState.Any,
			new[] {
				new EventData(
					Uuid.NewUuid(), SystemEventTypes.Settings,
	...
	public static async Task<ConditionalWriteResult> ConditionalAppendToStreamAsync(
		...
		try {
			var result = await client.AppendToStreamAsync(
					streamName, expectedRevision, eventData,
	...
	public static async Task<ConditionalWriteResult> ConditionalAppendToStreamAsync(
		...
		try {
			var result = await client.AppendToStreamAsync(
					streamName, expectedState, eventData,
					options => options.ThrowOnAppendFailure = false, deadline, userCredentials, cancellationToken
				)
				.ConfigureAwait(false);
