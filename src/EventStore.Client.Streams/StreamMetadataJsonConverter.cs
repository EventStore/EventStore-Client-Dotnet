...
class StreamMetadataJsonConverter : JsonConverter<StreamMetadata> {
	...
	public override StreamMetadata Read(
	...
	public override void Write(Utf8JsonWriter writer, StreamMetadata value, JsonSerializerOptions options) {
		...
		if (value.MaxCount.HasValue) writer.WriteNumber(SystemMetadata.MaxCount, value.MaxCount.Value);

		if (value.MaxAge.HasValue) writer.WriteNumber(SystemMetadata.MaxAge, (long)value.MaxAge.Value.TotalSeconds);

		if (value.TruncateBefore.HasValue) writer.WriteNumber(SystemMetadata.TruncateBefore, value.TruncateBefore.Value.ToInt64());

		if (value.CacheControl.HasValue) writer.WriteNumber(SystemMetadata.CacheControl, (long)value.CacheControl.Value.TotalSeconds);
