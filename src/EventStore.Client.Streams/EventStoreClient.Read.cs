...
namespace EventStore.Client {
	public partial class EventStoreClient {
		...
		public class ReadStreamResult : IAsyncEnumerable<ResolvedEvent>, IAsyncEnumerator<ResolvedEvent> {
			...
			public ReadStreamResult(
				...
				async Task<ReadState> GetStateInternal() {
					...
					return _call.Current?.ContentCase switch {
						ReadResp.ContentOneofCase.StreamNotFound => ReadState.StreamNotFound,
						_ => ReadState.Ok
					};
				}
			...
			public Task<ReadState> State { get; }
			...
			private static Exception ExceptionFromState(ReadState state, string streamName) {
				return state switch {
					ReadState.StreamNotFound => new StreamNotFoundException(streamName),
			...
			static (SubscriptionConfirmation, Position?, ResolvedEvent) ConvertToItem(ReadResp response) =>
