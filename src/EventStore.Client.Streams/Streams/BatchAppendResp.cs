...
partial class BatchAppendResp {
	public IWriteResult ToWriteResult() => ResultCase switch {
		...
		ResultOneofCase.Error => Error.Details switch {
			...
			{ } when Error.Details.Is(Timeout.Descriptor)      => throw new RpcException(new Status(StatusCode.DeadlineExceeded, Error.Message)),
			...
			{ } when Error.Details.Is(MaximumAppendSizeExceeded.Descriptor) =>
				throw new MaximumAppendSizeExceededException(Error.Details.Unpack<MaximumAppendSizeExceeded>().MaxAppendSize),
			{ } when Error.Details.Is(BadRequest.Descriptor) => throw new InvalidOperationException(
				Error.Details
	...
	static WrongExpectedVersionResult FromWrongExpectedVersion(
