...
public readonly struct ConditionalWriteResult : IEquatable<ConditionalWriteResult> {
	...
	/// </summary>
	public static readonly ConditionalWriteResult StreamDeleted = new(StreamRevision.None, Position.End, ConditionalWriteStatus.StreamDeleted);
	...
	ConditionalWriteResult(
		StreamRevision nextExpectedStreamRevision, Position logPosition,
	...
	internal static ConditionalWriteResult FromWriteResult(IWriteResult writeResult) =>
        writeResult switch {
			WrongExpectedVersionResult wrongExpectedVersion =>
				new ConditionalWriteResult(
					wrongExpectedVersion.NextExpectedStreamRevision, Position.End,
    ...
    internal static ConditionalWriteResult FromWrongExpectedVersion(WrongExpectedVersionException ex) =>
        new(ex.ExpectedStreamRevision, Position.End, ConditionalWriteStatus.VersionMismatch);
